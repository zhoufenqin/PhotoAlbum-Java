...
1. **Check Project Type and Prerequisites**:
   - **For Java projects**: Ensure Node.js is available, since the assessment runs through the bundled `run-java-appcat.js` script.
     - Check with `node --version`.
     - Node.js is normally already present: GitHub Copilot CLI and Claude Code are themselves Node.js applications, and standard GitHub-hosted runners ship Node.js. If for some reason `node` is **not** on PATH, install it before continuing (prefer `sudo` only when it is available and you are not already root):
       - Linux (Debian/Ubuntu): `apt-get update && apt-get install -y nodejs` (prefix with `sudo` if available)
       - macOS: `brew install node`
       - Windows: `winget install -e --id OpenJS.NodeJS`
     - The script only uses Node.js built-in modules — no `npm install` is required.
     - Archive extraction uses the system `tar` (present on Linux, macOS, and Windows 10+), so no extra tooling is needed.
     - No MCP tools are required for Java assessment.
...
2. **Run Assessment**:
   - **For Java projects**: Run the bundled `run-java-appcat.js` script (located in this skill's directory). It downloads the correct AppCAT CLI for the current OS/architecture from `appcat-java-manifest.json` (verifying its SHA-256), reads the assessment config to build the AppCAT analysis arguments, runs `appcat analyze`, injects assessment metadata into `report.json`, and writes the versioned report.
     - Run: `node <skill-dir>/run-java-appcat.js --workspace-path {workspace-path}`
     - The script auto-selects the AppCAT version/binary by OS and architecture (Linux/macOS/Windows, amd64/arm64).
     - The script reads `{workspace-path}/.github/modernize/assessment/reports/assessment-config.yaml` (falling back to built-in defaults when absent) and derives the `--label-selector` and analysis mode from it. Do **not** pre-edit the config to strip the `security` domain — the script automatically excludes `security` from AppCAT analysis while preserving it in the report metadata.
     - On success, the script prints the absolute path of the generated versioned `report.json`.
...
**For Java projects**:
- Node.js must be available (used to run the bundled `run-java-appcat.js` script). It is normally already present (Copilot CLI / Claude Code are Node.js apps; standard GitHub-hosted runners ship Node.js); otherwise install it (`apt-get`/`brew`/`winget`). For custom runners, you can also pre-install it via `copilot-setup-steps.yml`. The script uses only Node.js built-in modules — no `npm install` needed.
- No JDK is required — the appcat-for-java CLI bundles its own Java runtime.
- No MCP tools required — the script downloads and runs AppCAT directly using the bundled `appcat-java-manifest.json` (with SHA-256 verification).
...
- **For Java**: Verify Node.js is installed
  - Check with `node --version`
  - Install Node.js if missing (`apt-get`/`brew`/`winget`); no JDK is needed since the appcat-for-java CLI bundles its own runtime
